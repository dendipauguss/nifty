...
(function() {
    ...
    miniNavContentsCollapse = miniNavContents.map((collapseEl) => {
        ...
        if (activeToggler && (!isMiniNav || window.innerWidth < 992)) {
            ...
            let clp = new bootstrap.Collapse(collapseEl, {
                toggle: true
            });
            collapseEl.addEventListener("transitionend", () => parent.classList.remove("open"), {
                once: true
            });
            return clp;
        }
...
const toggleContent = (e) => {
    ...
    ["click", "touchend"].forEach(evt => document.addEventListener(evt, hideAllMiniNavContent, {
        once: true
    }));
};
